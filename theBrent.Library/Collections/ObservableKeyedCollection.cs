...
namespace theBrent.Library.Collections
{
        ...
        INotifyCollectionChanged, INotifyPropertyChanged where TKey : notnull
    {
        ...
        public int IndexOfKey(TKey key) => Contains(key) ? IndexOf(this[key]) : -1;
        ...
        {
            return _getKeyForItemDelegate != null ? _getKeyForItemDelegate(item) : 
                (keyPropertyName != null) ? (TKey)item!.GetType().GetProperty(keyPropertyName)!.GetValue(item)!:
                throw new ArgumentException(@"getKeyForItemDelegate and KeyPropertyName cannot both be null.");
        }
